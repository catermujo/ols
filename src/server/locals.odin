...
get_locals_stmt :: proc(
	...
	case ^ast.When_Stmt:
		...
		register_when_consts_from_globals(&when_expr_map, file, ast_context.globals)
		if stmt, ok := get_when_block_stmt(file, v, when_expr_map); ok {
	...
	case ^ast.With_Stmt:
		get_locals_stmt(file, v.init, ast_context, document_position)
		get_locals_stmt(file, v.opener, ast_context, document_position)
		get_locals_stmt(file, v.body, ast_context, document_position)
	case:
