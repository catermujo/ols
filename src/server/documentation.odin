...
directive_docs: map[string]string = {
	...
	"no_copy"                  = "```odin\n#no_copy\n```\n\nThis tag can be applied to a `struct`. Prevents trivial copying by disallowing implicit value copies of the struct.",
	...
	"trigger_location"         = "```odin\n#trigger_location\n```\n\n`#trigger_location` may only be used as an additional argument to `#assert` or `#panic`. It produces a `runtime.Source_Code_Location` for the use or import site that triggered the compile-time failure, rather than only the directive site.",
